...
router.post('/generate-post', auth, async (req, res) => {
  const { topic, tone, category, length, sourceUrl, useWebSearch, contentType, products } = req.body;
  if (!topic && !sourceUrl) return res.status(400).json({ error: 'Provide a topic or a source URL' });
  try {
    const postData = await ai.generatePost(topic, tone, category, { length, sourceUrl, useWebSearch, contentType, products });
...
router.post('/reedit-post', auth, async (req, res) => {
  const { postId, instructions, contentType } = req.body;
  try {
    const post = await Post.findById(postId);
    if (!post) return res.status(404).json({ error: 'Post not found' });
    const improved = await ai.reeditPost(post.content, post.title, instructions, contentType);
